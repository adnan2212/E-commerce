...
export const generateMetadata = async ({
  ...
  // TODO: need to fetch data from db
  return {
    title: product.name,
  ...
  return (
    <div className="flex flex-col gap-4 lg:flex-row md:gap-12 mt-12">
      ...
      <div className="w-full lg:w-7/12 flex flex-col gap-4">
        ...
        <h2 className="text-2xl font-semibold">₹{product.price.toLocaleString('en-IN')}</h2>
        <ProductInteraction product={product} selectedSize={selectedSize} selectedColor={selectedColor}/>
        {/* CARD INFO */}
        <div className="flex items-center gap-2 mt-4">
          <Image src="/cards.png" alt="klarna" width={50} height={50} className="rounded-md" />
          <Image src="/stripe.png" alt="klarna" width={50} height={50} className="rounded-md" />
        </div>
        <p className="text-gray-500 text-xs">
          ...
          <span className="undeline hover:text-black">Refund Policies</span>.
        </p>
      </div>
    </div>
  )
